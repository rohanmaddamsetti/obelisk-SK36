...
Does increasing Obelisk copy number increase Obelisk stability, at least transiently?
...
I built a simple ODE model to examine how Obelisk copy number affects Obelisk stability.

**Model Assumptions**  

The population is modeled as a vector of two states, $\mathbf{x}$, where $x_1$ represents a subpopulation contain intracellular Obelisks, and $x_2$ represents a subpopulation that has lost the Obelisks.

$\mathbf{x}(t) = \begin{pmatrix} x_1 \\ x_2 \end{pmatrix}$  

The total population size $X(t) = x_1(t) + x_2(t)$.  
...
We assume that subpopulation $x_1$ grows at rate $r_1$, and subpopulation $x_2$ grows at rate $r_2$. We use the term "fitness" synonymously with these growth rates $r_i$.

We assume that $r_1 = r_2 - \alpha N$, where $N$ is the Obelisk copy number per cell.

This assumption means that Obelisks have no fitness benefit, and confer a small linear fitness cost $\alpha$ per copy. 

We define $N^*$ as the Obelisk capacity; that is, the Obelisk copy number at which $r_1 = 0$. So,
...
In our model, Obelisk copy number $N$ and Obelisk capacity $N^*$ are free parameters, under the assumption that $0 \leq N < N^*$.
...
We assume that $x_1$ cells lose their Obelisks at rate $\mu$.

We model $\mu$ as a geometrically decreasing function of Obelisk copy number $N$, based on the following argument:

Suppose a cell with Obelisks divides. The per capita rate of generating a cell without Obelisks is the probability that one daughter gets no Obelisks and the other gets all of its parent's Obelisks. We assume partitioning is purely stochastic. So, the probability that a daughter doesn't get any Obelisks is:

$\mu = (1/2)^N$

Since the per-capita growth rate for Type 1 cells is $r_1$, the per-capita rate of generating Obelisk-free daughter cells is $r_1\mu$, and the per-capita rate of generating daughter cells with Obelisks is $r_1 (1 – \mu)$.

*Full dynamics*  

The selection and mutation dynamics are combined into a matrix $\mathbf{A}$:

$\mathbf{A} = \begin{bmatrix}
r_1(1- \mu) & 0 \\
r_1\mu & r_2 \end{bmatrix}$  
...
Consider the initial condition $\mathbf{x}(0) = \begin{pmatrix} 1 \\ 0 \end{pmatrix}$.

Since we assume that $r_2 > r_1$, eventually $x_2 \ge x_1$. How long does it take for $x_2 = x_1$? This is a natural measure of the transient stability of the $x_1$ state.

We have a linear system of ODEs that can solved analytically.

$\frac{d\mathbf{x}}{dt} = \mathbf{A}\mathbf{x}(t)$

$\frac{d\mathbf{x}}{dt} = \begin{bmatrix}
r_1(1- \mu) & 0 \\
r_1\mu & r_2 \end{bmatrix}  \mathbf{x}(t)$

The eigenvalues of $\mathbf{A}$ are the diagonal entries $r_1(1-\mu)$ and $r_2$, as these are the roots of the characteristic polynomial of $\mathbf{A}$.

Solving for the corresponding eigenvectors, we find the following general solution:

$\mathbf{x}(t) = c_1\begin{bmatrix} -1 \\ \frac{r_1\mu}{r_2 - r_1(1-\mu)} \end{bmatrix}e^{r_1(1-\mu)t} + c_2\begin{bmatrix} 0 \\ 1 \end{bmatrix}e^{r_2t}$

Using the initial condition, $\mathbf{x}(0) = \begin{pmatrix} 1 \\ 0 \end{pmatrix}$, we find that:

$c_1 = -1$

$c_2 = \frac{r_1\mu}{r_2 - r_1(1-\mu)}$

So that the particular solution is:

$\mathbf{x}(t) = \begin{bmatrix} 1 \\ \frac{-r_1\mu}{r_2 - r_1(1-\mu)} \end{bmatrix}e^{r_1(1-\mu)t} + \begin{bmatrix} 0 \\ \frac{r_1\mu}{r_2 - r_1(1-\mu)} \end{bmatrix}e^{r_2t}$
...
$t^* = \frac{log(1 + \frac{r_2 - r_1(1-\mu)}{r_1\mu})}{r_2 - r_1(1-\mu)}$

where $log()$ is the natural logarithm (base $e$).

without loss of generality, we can let $r_2 = 1$, so that $r_1 = 1 - \frac{N}{N^*}$ and $\mu = (1/2)^N$,

and examine how $t^*$ changes as we change Obelisk copy number $N$ and Obelisk capacity $N^*$ (which defines the burden or fitness cost of an intracellular Obelisk population).
...
begin
	...
	R_2 = 1.0 ## define fitness for cells without obelisks.
	
	## define the fitness of cells containing obelisks
	R_1 = R_2 * (1 - (OBELISK_COPY_NUM/OBELISK_CAPACITY))

	## define the loss rate for cells containing obelisks
	μ = 0.5^OBELISK_COPY_NUM
...
function calc_tstar(N=OBELISK_COPY_NUM,Nmax=MAX_OBELISK_COPY_NUM)
	...
	r2 = 1.0 ## define fitness for cells without obelisks.
	## define the fitness of cells containing obelisks
	r1 = r2 * (1 - N/Nmax)
	## define the loss rate for cells containing obelisks
	M = 0.5^N

	## use the formula to calculate tstar given the initial condition.
	Z = r2 - r1*(1-M)
	tstar = log(1 + Z/(r1*M))/Z
...
let
	...
	Nmax_vec = [x for x in 1:1200]
	N_vec = [x for x in 1:1200]

	## fill the tstar_matrix.
	tstar_matrix = [NaN for i in N_vec, j in Nmax_vec]

	## Create arrays to store the Source Data for Figure 1H.
	N_column = Float64[]
	Nmax_column = Float64[]
	tstar_column = Float64[]

	for my_nmax in Nmax_vec
		for my_n in N_vec
			if my_n < my_nmax
				my_tstar = calc_tstar(Float64(my_n),Float64(my_nmax))
				## update tstar_matrix
				tstar_matrix[my_n, my_nmax] = my_tstar
				## Append data to the arrays for Source Data.
    			append!(N_column, my_n)
    			append!(Nmax_column, my_nmax)
    			append!(tstar_column, my_tstar)
			end
		end
	end

	## Create a DataFrame to store the Source Data
	Fig1H_source_data_df = DataFrame(
		ObeliskCapacity = Nmax_column,
    	ObeliskCopyNumber = N_column,
   		response_time = tstar_column
	)
	## Save Fig1H Source Data to file.
	CSV.write("../results/Fig1H-Source-Data.csv", Fig1H_source_data_df)

	## Make Figure 1H.
	fig1H = heatmap(
		tstar_matrix, 
		ylabel="Obelisk Copy Number", 
		xlabel="Obelisk Capacity", 
		title="Time for Obelisk-free cells to reach\n50% population frequency", 
	...
	# Customize font size
	plot!(fig1H, titlefontsize=14, xlabelfontsize=12, ylabelfontsize=12, tickfontsize=12)

	# Draw a dashed diagonal line
	plot!([1, 1200], [1, 1200], line=:dash, color=:red, legend=false)
	
	savefig(fig1H, "../results/Fig1H-tstar-heatmap.pdf")
	fig1H
...
begin
	...
	prob = ODEProblem(odefunc, initial_pop_vec, tspan)
...
let
	...
	## plot the normalized first component over time
	x_1 = subpopulation_time_courses[1] ./ total_popsize_over_time
	timecourse_plot = plot(final_t1, x_1, xlabel="Time", ylabel="Frequency",
		grid=false, fontfamily="Helvetica",legend=false,size=(3.5*72,3*72))

	## plot the normalized second component over time
	x_2 = subpopulation_time_courses[2] ./ total_popsize_over_time
	plot!(timecourse_plot, final_t1, x_2, label=nothing)

	savefig(timecourse_plot, "../results/Fig1E-ExampleTimeCourse.pdf")
	timecourse_plot
end
...
function dynamics!(du, u, p, t)	
	...
	## get the parameters.
	obelisk_copy_number, obelisk_capacity = p
		
	r2 = 1.0 ## define fitness for cells without obelisks.
	
	## define the fitness of cells containing obelisks
	r1 = r2 * (1 - (obelisk_copy_number/obelisk_capacity))

	## define the loss rate for cells containing obelisks
	mu = 0.5^obelisk_copy_number
...
function TimeSeriesLostObeliskPopFrequency(obelisk_copy_number, obelisk_capacity)
	...
	LostObelisk_TimeSeries = LostObeliskPopFrequencyTimeSeries(my_sol.t, LostObeliskPopFrequencyOverTime(my_sol))
...
let
	...
	time_data = Float64[]
	lost_obelisk_pop_frequency_data = Float64[]
	obelisk_copy_number_data = Float64[]
	obelisk_capacity_data = Float64[]
	
	for obelisk_capacity in [20, 200, 400, 600]
		LostObelisk_timeseries = TimeSeriesLostObeliskPopFrequency(OBELISK_COPY_NUM, obelisk_capacity)

		## Append data to the arrays for Source Data.
    	append!(time_data, LostObelisk_timeseries.t)
    	append!(lost_obelisk_pop_frequency_data, LostObelisk_timeseries.v)
    	append!(obelisk_copy_number_data, fill(OBELISK_COPY_NUM,length(LostObelisk_timeseries.t)))
		
		append!(obelisk_capacity_data, fill(obelisk_capacity,length(LostObelisk_timeseries.t)))
		
		my_label = "Obelisk capacity = $obelisk_capacity"
		plot!(LostObelisk_timeseries.t, LostObelisk_timeseries.v, label=my_label,
			legend = false,
			ylabel="Frequency of cells\nwithout Obelisks",
	...
	## Create a DataFrame to store the Source Data
	Fig1F_source_data_df = DataFrame(
    	Time = time_data,
   		LostObeliskPopFrequency = lost_obelisk_pop_frequency_data,
		ObeliskCopyNumber = obelisk_copy_number_data,
    	ObeliskCapacity = obelisk_capacity_data
	)
	## Save Fig1F Source Data to file.
	CSV.write("../results/Fig1F-Source-Data.csv", Fig1F_source_data_df)
	
	savefig(fig1F, "../results/Fig1F-Dynamics_varying_ObeliskCapacity.pdf")
	fig1F
end
...
let
	...
	for obelisk_copy_number in [10, 20, 100, 800]
		LostObelisk_timeseries = TimeSeriesLostObeliskPopFrequency(obelisk_copy_number, OBELISK_CAPACITY)

		## Append data to the arrays for Source Data.
    	append!(time_data, LostObelisk_timeseries.t)
    	append!(lost_obelisk_pop_frequency_data, LostObelisk_timeseries.v)
    	append!(obelisk_copy_number_data, fill(obelisk_copy_number,length(LostObelisk_timeseries.t)))
		
		append!(obelisk_capacity_data, fill(OBELISK_CAPACITY,length(LostObelisk_timeseries.t)))
		
		my_label = "Obelisk copy number = $obelisk_copy_number"
		plot!(LostObelisk_timeseries.t, LostObelisk_timeseries.v, label=my_label,
			legend = false,
			ylabel="Frequency of cells\nwithout Obelisks",
			palette = :Hokusai3, #:tol_bright,
	...
	## Create a DataFrame to store the Source Data
	Fig1G_source_data_df = DataFrame(
    	Time = time_data,
   		LostObeliskPopFrequency = lost_obelisk_pop_frequency_data,
		ObeliskCopyNumber = obelisk_copy_number_data,
    	ObeliskCapacity = obelisk_capacity_data
	)
	## Save Fig1G Source Data to file.
	CSV.write("../results/Fig1G-Source-Data.csv", Fig1G_source_data_df)
	
	savefig(fig1G, "../results/Fig1G-Dynamics_varying_ObeliskCopyNumber.pdf")
	fig1G
end
